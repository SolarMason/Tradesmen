...
│   ├── stripe_bulk_create.py   ← Create 135 Products + Prices + Payment Links via Stripe API
...
The site has **no backend** — all checkout logic runs through Stripe Payment Links. Each of the 135 trade × tier × duration SKUs gets one Payment Link. To avoid clicking through the Stripe dashboard 135 times, the repo includes a one-shot bulk-creation script.

### Required Payment Link configuration (per SKU)

Each Payment Link is created with this exact configuration:

| Setting | Value |
|---|---|
| **Product** | One per SKU. Name format: *"Journeyman Electrician — Full Day (8h)"* |
| **Description** | Explains the booking covers WC, payroll, GL, dispatch, and that the tradesperson deploys to the shipping address |
| **Price** | One-time, USD, computed as `rate × hours × multiplier` |
| **Shipping address collection** | ✅ **Required** (US) — *this is the JOBSITE* |
| **Phone collection** | ✅ Required |
| **Custom field 1** | *"Project description / scope of work"* — Required, 10–500 chars |
| **Custom field 2** | *"On-site contact name & cell phone"* — Required, 5–100 chars |
| **Custom field 3** | *"Access notes (gate codes, parking, supervisor, etc.)"* — Optional, ≤500 chars |
| **Custom text on shipping** | *"This is the JOBSITE — where the tradesperson will be dispatched on your start date."* |
| **After payment** | Redirect → `https://tradesmen.nepa-pro.com/?booked=1` |
| **Promotion codes** | Allowed |
| **Metadata** | `sku`, `trade`, `tier`, `duration`, `category`, `hours`, `billable_hourly_usd` (for reconciliation in dashboard reports) |

### Bulk-create script (creates all 135 in one run)

```bash
# 1. Install Stripe SDK (one time)
pip install stripe

# 2. Export your TEST secret key first
export STRIPE_SECRET_KEY=sk_test_...

# 3. Dry run (no API calls — just validates)
python3 _build/stripe_bulk_create.py --dry-run

# 4. Test with one SKU first (recommended)
python3 _build/stripe_bulk_create.py --only electricians_journeyman_full

# 5. Run for real (creates all 135 in test mode)
python3 _build/stripe_bulk_create.py

# 6. After verifying in Stripe dashboard, switch to LIVE and re-run
export STRIPE_SECRET_KEY=sk_live_...
python3 _build/stripe_bulk_create.py
```

The script:
- ✅ Reads `skus.csv` → creates 135 Products + Prices + Payment Links
- ✅ Configures **every field** from the table above (no manual dashboard config)
- ✅ Saves results to `stripe_links.json` after each SKU (resumable on failures)
- ✅ Patches `index.html`'s `STRIPE_LINKS` lookup with all 135 link IDs automatically
- ✅ Idempotent — re-running skips SKUs already created
- ✅ Confirms before live mode (avoids accidental real-money runs)
- ✅ Backs off on rate limits

When done, every "Book & Pay" button in the site routes to the right fully-configured Stripe Payment Link. Commit the patched `index.html` and push — done.

### What happens until you run the script
...
Every successful Stripe checkout fires with:
- `client_reference_id` = `{sku}_{total}` (visible in dashboard payment record)
- Custom `metadata` on Product, Price, Payment Link, and Checkout Session (sku, trade, tier, duration, category)

That's how you match a payment back to the trade/tier/duration the contractor selected, and how you build dispatch reports per category.
